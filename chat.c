 ...
 * NULL, must have its last pointer be NULL to denote its end.  We also require
 * that the message is a NULL terminated string.  If ensurenewline is non-zero, then
...
void* recvMsg(void*)
{
    ...
    while (1) {
        ...
        char* m = (char*)malloc(maxlen+2);
        memcpy(m, msg, nbytes);
        if (m[nbytes-1] != '\n')
            m[nbytes++] = '\n';
        m[nbytes] = 0;
        g_main_context_invoke(NULL, shownewmessage, m);
    }
    return 0;
}
